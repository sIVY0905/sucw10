...
class ChoreManager(models.Manager):
    ...
    def get_due_date(self, chore):
        ...
        # last_completed 可能是 datetime.datetime 對象，我們轉為 date
        last_completed_date = chore.last_completed if chore.last_completed else None
        ...
        # 應完成日期 = 上次完成日期 + 頻率
        return last_completed_date + timedelta(days=chore.frequency_days)
    ...
    def get_status(self, chore):
        ...
        # 2. 當天應完成或逾期 1 天 (Green - 寬限期)
        if days_diff >= 0 and days_diff <= 1:
            return 'Green'
    ...
    def get_chore_list_data(self, room):
        ...
        """
        # 預載入 records 和 assigned_to 關係，減少資料庫查詢
        chores = self.filter(room=room).prefetch_related('records', 'assigned_to').order_by('type', 'private_area', 'title')
        ...
        for chore in chores:
            ...
            # 使用 .first() 可以避免在 Python 中使用 [0] 檢查空列表
            last_record = chore.records.order_by('-completed_on').first()
            last_completed_date = last_record.completed_on if last_record else None
                
            days_ago = None
            if last_completed_date:
                days_ago = (today - last_completed_date.date()).days
    ...
    def get_completion_percentage(self, room):
        ...
        for chore in all_chores:
            ...
            # 'Done' (今天完成) 或 'Grey' (未來預訂) 視為完成。
            # 只有 'Green' (當天/逾期1天) 或 'Red' (逾期2天+) 且非'Done' 視為待辦/積欠。
            if status == 'Done' or status == 'Grey':
                    completed_count += 1
